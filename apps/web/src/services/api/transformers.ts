...
const extractApiKeyValue = (payload: unknown): string => {
  if (typeof payload === 'string') return payload.trim();
  if (!isRecord(payload)) return '';
  return String(payload['api-key'] ?? payload.apiKey ?? payload.key ?? '').trim();
};
...
const normalizeApiKeyAccessRules = (payload: unknown): Config['apiKeyAccessRules'] => {
  ...
  const rules = payload
    .map((item) => {
      ...
      const allowedAuthIndexes = normalizeExcludedModels(
        record['allowed-auth-indexes'] ?? record.allowedAuthIndexes ?? record.allowed_auth_indexes
      );
      const allowedAuthIds = normalizeExcludedModels(
      ...
      return {
        apiKey,
        ...(allowedAuthIndexes.length ? { allowedAuthIndexes } : {}),
        ...(allowedAuthIds.length ? { allowedAuthIds } : {}),
      };
    })
    .filter(Boolean) as NonNullable<Config['apiKeyAccessRules']>;

  return rules.length ? rules : undefined;
};

const mergeApiKeyAccessRules = (
  base?: Config['apiKeyAccessRules'],
  overrides?: Config['apiKeyAccessRules']
): Config['apiKeyAccessRules'] => {
  const merged = [...(base ?? [])];
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  if (Array.isArray(apiKeysRaw)) {
    config.apiKeys = apiKeysRaw.map((key) => extractApiKeyValue(key)).filter(Boolean);
  }
  const apiKeyAccessRules = mergeApiKeyAccessRules(
    normalizeApiKeyAccessRules(raw['api-key-access-rules'] ?? raw.apiKeyAccessRules),
    normalizeApiKeyAccessRules(apiKeysRaw)
  );
  if (apiKeyAccessRules) {
    config.apiKeyAccessRules = apiKeyAccessRules;
  }
